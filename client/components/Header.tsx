...
export const Header: React.FC<HeaderProps> = ({ transparent: propsTransparent }) => {
  ...
  const navContainerClasses = "relative hidden lg:flex items-center gap-1 rounded-full bg-white/5 dark:bg-black/20 px-1.5 py-1.5 shadow-inner ring-1 ring-white/10 dark:ring-white/5 backdrop-blur-xl transition-all duration-500";

  // Updated Header background logic: Ultra-thin glassmorphism without harsh borders
  const headerBgClass = (isTransparent && !scrolled)
    ? "bg-transparent py-3"
    : "bg-white/40 dark:bg-[#050a12]/40 backdrop-blur-md shadow-2xl dark:shadow-black/20 py-3";

  return (
    <header className={`fixed top-0 z-50 w-full transition-all duration-500 ${headerBgClass}`}>
      <div className="relative mx-auto flex h-14 max-w-7xl items-center justify-between px-6">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-30 hidden lg:block">
          <nav className={navContainerClasses}>
            ...
            {NAV_ITEMS.filter(item => item.path !== '/admin' || isAuthenticated).map((item, index) => {
